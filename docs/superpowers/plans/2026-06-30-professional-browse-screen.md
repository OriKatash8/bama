# Professional Browse Screen Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Add a Browse tab to professional mode so professionals can discover other professionals by skill category, excluding themselves from results.

**Architecture:** Create one new screen file (`browse/index.tsx`) that mirrors the client browse screen with a self-exclusion filter; register it as a new tab in the professional layout. No shared hooks or components are modified.

**Tech Stack:** React Native, Expo Router (file-based routing), Firebase Auth (`auth.currentUser`), lucide-react-native icons.

## Global Constraints

- Follow the existing tab registration pattern in `src/app/(professional)/(tabs)/_layout.tsx`
- Icon strokeWidth must be `1.5` (professional layout convention — client uses `2.5`)
- No changes to `useSearchProfessionals`, `ProfessionalCard`, or any shared feature code
- Self-exclusion is client-side only — filter after the hook resolves, before rendering

---

### Task 1: Create the professional browse screen

**Files:**
- Create: `src/app/(professional)/(tabs)/browse/index.tsx`
- Modify: `src/app/(professional)/(tabs)/_layout.tsx`

**Interfaces:**
- Consumes: `useSearchProfessionals(category, subcategory)` → `{ results: ProfessionalResult[], isLoading: boolean }` from `@features/crew/hooks`
- Consumes: `ProfessionalCard` from `@features/crew/components` — props: `{ item: ProfessionalResult, onMessage?: () => Promise<void> }`
- Consumes: `auth.currentUser?.uid` from `@core/firebase/config` for self-exclusion
- Consumes: `getOrCreateDM(currentUserId, professionalId)` → `Promise<string>` from `@features/chat/services/chatService`

- [ ] **Step 1: Create the browse screen file**

Create `src/app/(professional)/(tabs)/browse/index.tsx` with the full content below:
...
function ResultsView({ category, subcategory }: { category: string; subcategory: string }) {
  ...
  const filtered = results.filter((r) => r.user.id !== currentUid);
      ...
      <View style={styles.container}>
        <View style={styles.header}>
          ...
          <Text style={[styles.heading, { color: colors.text }, gradientText]}>
            {view.kind === 'grid' ? 'Browse Professionals' : (view as any).subcategory}
          </Text>
...
- [ ] **Step 2: Register the browse tab in the professional layout**

Open `src/app/(professional)/(tabs)/_layout.tsx`. Add `Search` to the lucide import and insert the `browse` tab between `marketplace` and `chats`:
...
Then add the Tabs.Screen entry between `marketplace` and `chats`:
...
Start the app and log in as a professional. Check:

1. A Search icon tab appears in the professional tab bar between Marketplace and Chats
2. Tapping it shows the "Browse Professionals" heading and accordion category list
3. Expanding a category and tapping a subcategory shows a results list
4. Your own profile card does NOT appear in any results list
5. Tapping "Message" on another professional's card navigates to `(professional)/chats/<chatId>` (not the client route)
6. Searching by keyword (e.g., "director") shows the inline results view with the hint banner
7. Clearing the search returns to the accordion list

- [ ] **Step 4: Commit**

```bash
git add src/app/\(professional\)/\(tabs\)/browse/index.tsx src/app/\(professional\)/\(tabs\)/_layout.tsx
git commit -m "feat: add browse screen to professional mode with self-exclusion filter"
```
